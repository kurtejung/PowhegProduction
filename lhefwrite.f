...
c...writes event information to a les houches events file on unit nlf. 
      ...
      include 'pwhg_flg.h'
      integer i,j
c...add a filter to only write events with a particle above pT=50
      REAL transmom,temppt
      transmom=0
      do 190 i=1,nup
        temppt = sqrt(pup(1,i)*pup(1,i)+pup(2,i)*pup(2,i))
        if(temppt.GT.transmom) transmom = temppt
 190  continue
      
      IF(transmom.GT.50) THEN
          write(nlf,'(a)')'<event>'
